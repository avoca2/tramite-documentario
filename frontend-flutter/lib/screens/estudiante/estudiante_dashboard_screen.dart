...
class EstudianteDashboardScreen extends StatefulWidget {
  ...
  @override
  State<EstudianteDashboardScreen> createState() => _EstudianteDashboardScreenState();
}
...
class _EstudianteDashboardScreenState extends State<EstudianteDashboardScreen> {
  ...
  Future<void> _cargarDashboardStats() async {
    ...
    try {
      ...
      setState(() {
        ...
        _misTramites = data['ultimos_tramites'] ?? [];
        _loadingStats = false;
      });
    } catch (e) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final misTramites = _misTramites.length;
    final tramitesPendientes = _misTramites.where((t) => t['estado'] == 'pendiente').length;
    final tramitesActivos = _misTramites.where((t) => t['estado'] == 'activo').length;
                      ...
                      children: [
                        ...
                        GridView.count(
                          ...
                          children: [
                            ...
                            StatCard(
                              title: '$misTramites',
                              subtitle: 'Mis Tramites\nTotales',
                              color: Colors.purple,
                              icon: Icons.assignment,
                            ),
                            StatCard(
                              title: '$tramitesPendientes',
                              subtitle: 'Tramites\nPendientes',
                              color: Colors.orange,
                              icon: Icons.pending,
                            ),
                            StatCard(
                              title: '$tramitesActivos',
                              subtitle: 'Tramites\nActivos',
                              color: Colors.green,
                              icon: Icons.check_circle,
                            ),
                            StatCard(
                              title: '${authProvider.user?.name?.split(' ').first ?? ''}',
                              subtitle: 'Mi Perfil\nEstudiante',
                              color: Colors.indigo,
                              icon: Icons.person,
                            ),
    ...
    return Container(
      ...
      child: Column(
        children: [
            ...
            child: Row(
              children: [
                CircleAvatar(
                  ...
                  child: Text(
                    authProvider.user?.name?.substring(0, 1).toUpperCase() ?? 'E',
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        authProvider.user?.name ?? 'Estudiante',
          ...
          const Divider(),
          _buildSidebarItem(Icons.dashboard, 'Inicio', () {}),
          _buildSidebarItem(Icons.school, 'Mi Matricula', () {
            Navigator.push(context, MaterialPageRoute(builder: (context) => const MatriculaScreen()));
          }),
          _buildSidebarItem(Icons.swap_horiz, 'Convalidacion', () {
            Navigator.push(context, MaterialPageRoute(builder: (context) => const ConvalidacionScreen()));
          }),
          _buildSidebarItem(Icons.grade, 'Titulacion', () {
            Navigator.push(context, MaterialPageRoute(builder: (context) => const TitulacionScreen()));
          }),
          _buildSidebarItem(Icons.assignment, 'Mis Tramites', () {
            Navigator.push(context, MaterialPageRoute(builder: (context) => const MisTramitesScreen()));
          }),
          const Divider(),
          _buildSidebarItem(Icons.person, 'Mi Perfil', () {
    ...
    return Row(
      children: [
        ...
        Column(
          ...
          children: [
            Text(
              'Bienvenido, ${authProvider.user?.name ?? "Estudiante"}',
              style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold),
            ),
            Text(
              'Gestiona tus tramites academicos de manera rapida y sencilla.',
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text(
              'Mis Tramites Recientes',
              style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 16),
            if (_misTramites.isEmpty)
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 20),
                child: Center(
                  child: Text('No tienes tramites registrados'),
                ),
              )
            else
              ..._misTramites.take(4).map((tramite) => _buildTramiteRow(tramite)),
            ...
            TextButton(
              ...
              child: const Text('Ver todos mis tramites →'),
            ),
          ],
        ),
      ),
  ...
  Widget _buildTramiteRow(dynamic tramite) {
    ...
    return ListTile(
      ...
      leading: CircleAvatar(
        ...
        child: Icon(
          _getTramiteIcon(tramite['tipo'] ?? ''),
          color: Colors.grey.shade700,
          size: 18,
        ),
      ),
      title: Text(
        tramite['titulo'] ?? 'Trámite',
        style: const TextStyle(fontWeight: FontWeight.w500),
      ),
      subtitle: Text('${tramite['fecha'] ?? ''}'),
      trailing: Chip(
        label: Text(
          estadoText,
  ...
  IconData _getTramiteIcon(String tipo) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text('Ingresa tu DNI para verificar tus datos'),
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                ActionButton('Titulacion', Icons.grade, Colors.purple.shade100, () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => const TitulacionScreen()));
                }),
                ActionButton('Mis Tramites', Icons.assignment, Colors.green.shade100, () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => const MisTramitesScreen()));
                }),
              ],
            ),
